...
namespace CodePen {
	...
	void CodeEditor::ProcessKeyEvent(KeyPressedEvent& e)
	{
		...
		{
			int key = e.GetKeyCode();
			if (key == PS_KEY_ENTER || key == PS_KEY_TAB)
			{
		...
		if ((mods & GLFW_MOD_CONTROL) && key == PS_KEY_C)
		{
			Copy();
			return;
		}
		if ((mods & GLFW_MOD_CONTROL) && key == PS_KEY_X)
		{
			Cut();
			return;
		}
		if ((mods & GLFW_MOD_CONTROL) && key == PS_KEY_V)
		{
			Paste();
			return;
		}
		if ((mods & GLFW_MOD_CONTROL) && key == PS_KEY_Z)
		{
			Undo();
			return;
		}
		if ((mods & GLFW_MOD_CONTROL) && key == PS_KEY_Y)
		{
			Redo();
			return;
		}
		if ((mods & GLFW_MOD_CONTROL) && key == PS_KEY_A)
		{
		...
		else if (key == PS_KEY_RIGHT)
		{
			m_Cursor.Move(0, 1, m_Buffer, (mods & GLFW_MOD_SHIFT) != 0);
		}
		else if (key == PS_KEY_UP)
		{
			m_Cursor.Move(-1, 0, m_Buffer, (mods & GLFW_MOD_SHIFT) != 0);
		}
		else if (key == PS_KEY_DOWN)
		{
			m_Cursor.Move(1, 0, m_Buffer, (mods & GLFW_MOD_SHIFT) != 0);
		}
		else if (key == PS_KEY_HOME)
		{
	...
	{
		if (e.GetMouseButton() != PS_MOUSE_BUTTON_LEFT) return;
